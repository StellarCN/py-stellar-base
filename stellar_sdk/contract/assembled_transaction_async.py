...
__all__ = ["AssembledTransactionAsync"]


class AssembledTransactionAsync(Generic[T]):
    ...
    async def prepare(self, restore: bool = True) -> "AssembledTransactionAsync":
        ...
        if (
            ...
            return await self.prepare(restore)
    ...
    async def authorize(
        ...
        *,
        ...
        valid_until_ledger_sequence = await self._resolve_valid_until_ledger_sequence(
        ...
        *,
        ...
        if valid_until_ledger_sequence is None and valid_for_ledger_count is None:
            ...
            valid_until_ledger_sequence = (
                await self.server.get_latest_ledger()
            ).sequence + 100
    ...
    async def _resolve_valid_until_ledger_sequence(
        self,
        ...
        if self.simulation is not None:
            ...
            latest_ledger = (await self.server.get_latest_ledger()).sequence
    ...
    async def _submit(self) -> GetTransactionResponse:
        if not self.built_transaction:
            raise NotYetSimulatedError("Transaction has not yet been simulated.", self)

        if not self.send_transaction_response:
            self.send_transaction_response = await self.server.send_transaction(
        ...
        self.get_transaction_response = (
            await _with_exponential_backoff(
    ...
    async def restore_footprint(self, restore_preamble: RestorePreamble) -> None:
        if not self.transaction_signer:
            raise ValueError(
                "For automatic restore to work you must provide a transaction_signer when initializing AssembledTransactionAsync."
            )
        restore_tx = (
            TransactionBuilder(
...
async def _with_exponential_backoff(
    fn,
    keep_waiting_if,
    timeout: float,
