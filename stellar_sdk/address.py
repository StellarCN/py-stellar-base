...
class Address:
    ...
    def to_xdr_sc_address(self) -> stellar_xdr.SCAddress:
        ...
        if self.type == AddressType.ACCOUNT:
            ...
            return stellar_xdr.SCAddress(stellar_xdr.SCAddressType.SC_ADDRESS_TYPE_ACCOUNT, account_id=account)
        elif self.type == AddressType.CONTRACT:
            contract = Hash(self.key)
            return stellar_xdr.SCAddress(stellar_xdr.SCAddressType.SC_ADDRESS_TYPE_CONTRACT, contract_id=contract)
        else:
            raise ValueError("Unsupported address type.")
    ...
    def to_xdr_sc_val(self) -> stellar_xdr.SCVal:
        return stellar_xdr.SCVal(stellar_xdr.SCValType.SCV_ADDRESS, address=self.to_xdr_sc_address())
