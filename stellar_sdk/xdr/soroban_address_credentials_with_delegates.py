...
class SorobanAddressCredentialsWithDelegates:
    ...
    def __init__(
        ...
        if delegates and len(delegates) > _expect_max_length:
            raise ValueError(f"The maximum length of `delegates` should be {_expect_max_length}, but got {len(delegates)}.")
    ...
    @classmethod
    def unpack(cls, unpacker: Unpacker, depth_limit: int = DEFAULT_XDR_MAX_DEPTH) -> SorobanAddressCredentialsWithDelegates:
        if depth_limit <= 0:
            raise ValueError("Maximum decoding depth reached")
        address_credentials = SorobanAddressCredentials.unpack(unpacker, depth_limit - 1)
        length = unpacker.unpack_uint()
        _remaining = len(unpacker.get_buffer()) - unpacker.get_position()
        if _remaining < length:
            raise ValueError(f"delegates length {length} exceeds remaining input length {_remaining}")
        delegates = []
    ...
    def from_json_dict(cls, json_dict: dict) -> SorobanAddressCredentialsWithDelegates:
        address_credentials = SorobanAddressCredentials.from_json_dict(json_dict["address_credentials"])
        delegates = [SorobanDelegateSignature.from_json_dict(item) for item in json_dict["delegates"]]
        return cls(
            address_credentials=address_credentials,
            delegates=delegates,
        )
    def __hash__(self):
        return hash((self.address_credentials, self.delegates,))
    def __eq__(self, other: object):
        if not isinstance(other, self.__class__):
            return NotImplemented
        return self.address_credentials == other.address_credentials and self.delegates == other.delegates
    def __repr__(self):
        out = [
