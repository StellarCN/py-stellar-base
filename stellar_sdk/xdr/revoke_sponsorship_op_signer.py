...
class RevokeSponsorshipOpSigner:
    ...
    def unpack(
        ...
        signer_key = SignerKey.unpack(unpacker, depth_limit - 1)
        return cls(
    ...
    @classmethod
    def from_json(cls, json_str: str) -> RevokeSponsorshipOpSigner:
        return cls.from_json_dict(json.loads(json_str))

    def to_json_dict(self) -> dict:
        return {
            "account_id": self.account_id.to_json_dict(),
            "signer_key": self.signer_key.to_json_dict(),
        }

    @classmethod
    def from_json_dict(cls, json_dict: dict) -> RevokeSponsorshipOpSigner:
        account_id = AccountID.from_json_dict(json_dict["account_id"])
        signer_key = SignerKey.from_json_dict(json_dict["signer_key"])
        return cls(
            account_id=account_id,
            signer_key=signer_key,
        )
