...
class LedgerEntryExtensionV1Ext:
    ...
    @classmethod
    def from_json(cls, json_str: str) -> LedgerEntryExtensionV1Ext:
        return cls.from_json_dict(json.loads(json_str))

    def to_json_dict(self):
        if self.v == 0:
            return "v0"
        raise ValueError(f"Unknown v in LedgerEntryExtensionV1Ext: {self.v}")

    @classmethod
    def from_json_dict(cls, json_value: str) -> LedgerEntryExtensionV1Ext:
        if json_value not in ("v0",):
            raise ValueError(
                f"Unexpected string '{json_value}' for LedgerEntryExtensionV1Ext, must be one of: v0"
            )
        v = int(json_value[1:])
        return cls(v=v)
