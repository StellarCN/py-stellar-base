...
class SorobanAuthorizationEntries:
    ...
    """
    def __init__(self, soroban_authorization_entries: List[SorobanAuthorizationEntry]) -> None:
        _expect_max_length = 4294967295
        if soroban_authorization_entries and len(soroban_authorization_entries) > _expect_max_length:
            raise ValueError(f"The maximum length of `soroban_authorization_entries` should be {_expect_max_length}, but got {len(soroban_authorization_entries)}.")
    ...
    def unpack(cls, unpacker: Unpacker) -> SorobanAuthorizationEntries:
        ...
        for _ in range(length):
            soroban_authorization_entries.append(SorobanAuthorizationEntry.unpack(unpacker))
