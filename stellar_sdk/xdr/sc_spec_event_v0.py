...
class SCSpecEventV0:
    ...
    def __init__(
        ...
        if prefix_topics and len(prefix_topics) > _expect_max_length:
            raise ValueError(f"The maximum length of `prefix_topics` should be {_expect_max_length}, but got {len(prefix_topics)}.")
        _expect_max_length = 50
        if params and len(params) > _expect_max_length:
            raise ValueError(f"The maximum length of `params` should be {_expect_max_length}, but got {len(params)}.")
        self.doc = doc
        self.lib = lib
        self.name = name
    ...
    def __hash__(self):
        return hash((self.doc, self.lib, self.name, self.prefix_topics, self.params, self.data_format,))
    def __eq__(self, other: object):
        if not isinstance(other, self.__class__):
            return NotImplemented
        return self.doc== other.doc and self.lib== other.lib and self.name== other.name and self.prefix_topics== other.prefix_topics and self.params== other.params and self.data_format== other.data_format
    def __repr__(self):
        out = [
            f'doc={self.doc}',
            f'lib={self.lib}',
