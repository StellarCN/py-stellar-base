...
class DependentTxCluster:
    ...
    def __init__(self, dependent_tx_cluster: List[TransactionEnvelope]) -> None:
        ...
        if dependent_tx_cluster and len(dependent_tx_cluster) > _expect_max_length:
            raise ValueError(f"The maximum length of `dependent_tx_cluster` should be {_expect_max_length}, but got {len(dependent_tx_cluster)}.")
    ...
    def __repr__(self):
        return f"<DependentTxCluster [dependent_tx_cluster={self.dependent_tx_cluster}]>"
