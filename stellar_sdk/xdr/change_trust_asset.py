...
class ChangeTrustAsset:
    ...
    def unpack(
        ...
        if type == AssetType.ASSET_TYPE_POOL_SHARE:
            liquidity_pool = LiquidityPoolParameters.unpack(unpacker, depth_limit - 1)
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ChangeTrustAsset:
    ...
    def to_json_dict(self):
        ...
        if self.type == AssetType.ASSET_TYPE_POOL_SHARE:
            ...
            return {"pool_share": self.liquidity_pool.to_json_dict()}
        raise ValueError(f"Unknown type in ChangeTrustAsset: {self.type}")

    @classmethod
    def from_json_dict(cls, json_value: str | dict) -> ChangeTrustAsset:
        if isinstance(json_value, str):
            if json_value not in ("native",):
                raise ValueError(
                    f"Unexpected string '{json_value}' for ChangeTrustAsset, must be one of: native"
                )
            type = AssetType.from_json_dict(json_value)
            return cls(type=type)
        if not isinstance(json_value, dict) or len(json_value) != 1:
            raise ValueError(
                f"Expected a single-key object for ChangeTrustAsset, got: {json_value}"
            )
        ...
        if key == "credit_alphanum12":
            ...
            return cls(type=type, alpha_num12=alpha_num12)
        if key == "pool_share":
            liquidity_pool = LiquidityPoolParameters.from_json_dict(
                json_value["pool_share"]
            )
            return cls(type=type, liquidity_pool=liquidity_pool)
        raise ValueError(f"Unknown key '{key}' for ChangeTrustAsset")
