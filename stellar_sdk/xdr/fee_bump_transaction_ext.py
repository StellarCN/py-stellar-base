...
class FeeBumpTransactionExt:
    ...
    @classmethod
    def from_json(cls, json_str: str) -> FeeBumpTransactionExt:
        return cls.from_json_dict(json.loads(json_str))

    def to_json_dict(self):
        if self.v == 0:
            return "v0"
        raise ValueError(f"Unknown v in FeeBumpTransactionExt: {self.v}")

    @classmethod
    def from_json_dict(cls, json_value: str) -> FeeBumpTransactionExt:
        if json_value not in ("v0",):
            raise ValueError(
                f"Unexpected string '{json_value}' for FeeBumpTransactionExt, must be one of: v0"
            )
        v = int(json_value[1:])
        return cls(v=v)
