...
class TransactionMetaV4:
    ...
    def unpack(cls, unpacker: Unpacker) -> TransactionMetaV4:
        ...
        soroban_meta = SorobanTransactionMetaV2.unpack(unpacker) if unpacker.unpack_uint() else None
        length = unpacker.unpack_uint()
        events = []
        for _ in range(length):
    ...
    def __hash__(self):
        return hash((self.ext, self.tx_changes_before, self.operations, self.tx_changes_after, self.soroban_meta, self.events, self.diagnostic_events,))
    def __eq__(self, other: object):
        if not isinstance(other, self.__class__):
            return NotImplemented
        return self.ext== other.ext and self.tx_changes_before== other.tx_changes_before and self.operations== other.operations and self.tx_changes_after== other.tx_changes_after and self.soroban_meta== other.soroban_meta and self.events== other.events and self.diagnostic_events== other.diagnostic_events
    def __repr__(self):
        out = [
            f'ext={self.ext}',
