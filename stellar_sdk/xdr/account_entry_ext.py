...
class AccountEntryExt:
    ...
    def __init__(
        self,
        v: int,
        v1: Optional[AccountEntryExtensionV1] = None,
    ) -> None:
        self.v = v
        self.v1 = v1
    ...
    def unpack(
        ...
        if v == 0:
            return cls(v=v)
        if v == 1:
            v1 = AccountEntryExtensionV1.unpack(unpacker, depth_limit - 1)
    ...
    @classmethod
    def from_json(cls, json_str: str) -> AccountEntryExt:
    ...
    def to_json_dict(self):
        ...
        raise ValueError(f"Unknown v in AccountEntryExt: {self.v}")

    @classmethod
    def from_json_dict(cls, json_value: str | dict) -> AccountEntryExt:
        if isinstance(json_value, str):
            if json_value not in ("v0",):
                raise ValueError(
                    f"Unexpected string '{json_value}' for AccountEntryExt, must be one of: v0"
                )
            v = int(json_value[1:])
            return cls(v=v)
        if not isinstance(json_value, dict) or len(json_value) != 1:
            raise ValueError(
                f"Expected a single-key object for AccountEntryExt, got: {json_value}"
            )
        key = next(iter(json_value))
        v = int(key[1:])
        if key == "v1":
            v1 = AccountEntryExtensionV1.from_json_dict(json_value["v1"])
            return cls(v=v, v1=v1)
        raise ValueError(f"Unknown key '{key}' for AccountEntryExt")
