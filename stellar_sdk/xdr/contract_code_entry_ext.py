...
class ContractCodeEntryExt:
    ...
    def unpack(
        ...
        if v == 0:
            return cls(v=v)
        if v == 1:
            v1 = ContractCodeEntryV1.unpack(unpacker, depth_limit - 1)
    ...
    @classmethod
    def from_json(cls, json_str: str) -> ContractCodeEntryExt:
    ...
    def to_json_dict(self):
        ...
        raise ValueError(f"Unknown v in ContractCodeEntryExt: {self.v}")

    @classmethod
    def from_json_dict(cls, json_value: str | dict) -> ContractCodeEntryExt:
        if isinstance(json_value, str):
            if json_value not in ("v0",):
                raise ValueError(
                    f"Unexpected string '{json_value}' for ContractCodeEntryExt, must be one of: v0"
                )
            v = int(json_value[1:])
            return cls(v=v)
        if not isinstance(json_value, dict) or len(json_value) != 1:
            raise ValueError(
                f"Expected a single-key object for ContractCodeEntryExt, got: {json_value}"
            )
        key = next(iter(json_value))
        v = int(key[1:])
        if key == "v1":
            v1 = ContractCodeEntryV1.from_json_dict(json_value["v1"])
            return cls(v=v, v1=v1)
        raise ValueError(f"Unknown key '{key}' for ContractCodeEntryExt")
