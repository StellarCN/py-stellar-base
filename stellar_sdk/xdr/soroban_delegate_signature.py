...
class SorobanDelegateSignature:
    ...
    def __init__(
        ...
        if nested_delegates and len(nested_delegates) > _expect_max_length:
            raise ValueError(
                f"The maximum length of `nested_delegates` should be {_expect_max_length}, but got {len(nested_delegates)}."
            )
    ...
    def unpack(
        ...
        if _remaining < length:
            raise ValueError(
                f"nested_delegates length {length} exceeds remaining input length {_remaining}"
            )
        nested_delegates = []
        for _ in range(length):
            nested_delegates.append(
                SorobanDelegateSignature.unpack(unpacker, depth_limit - 1)
            )
        return cls(
            address=address,
            signature=signature,
    ...
    def from_json_dict(cls, json_dict: dict) -> SorobanDelegateSignature:
        ...
        nested_delegates = [
            ...
            for item in json_dict["nested_delegates"]
        ]
        return cls(
            address=address,
            signature=signature,
    ...
    def __eq__(self, other: object):
        ...
        return (
            ...
            and self.nested_delegates == other.nested_delegates
        )
