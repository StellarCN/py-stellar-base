...
class ParallelTxsComponent:
    ...
    def __init__(
        ...
        if execution_stages and len(execution_stages) > _expect_max_length:
            raise ValueError(f"The maximum length of `execution_stages` should be {_expect_max_length}, but got {len(execution_stages)}.")
    ...
    def __hash__(self):
        return hash((self.base_fee, self.execution_stages,))
    def __eq__(self, other: object):
        if not isinstance(other, self.__class__):
            return NotImplemented
        return self.base_fee== other.base_fee and self.execution_stages== other.execution_stages
    def __repr__(self):
        out = [
