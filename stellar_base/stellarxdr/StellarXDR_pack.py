# Generated by rpcgen.py from StellarXDR.x on Mon Nov  9 07:52:58 2015
...
class STELLARXDRPacker(xdrlib.Packer):
    ...
    def pack_CryptoKeyType(self, data):
        ...
        if self.check_enum and data not in [const.KEY_TYPE_ED25519]:
    ...
    def pack_SCPStatement(self, data):
        ...
        if data.pledges.type == const.SCP_ST_PREPARE:
            ...
            if data.pledges.prepare.nP is None:
                raise TypeError( 'data.pledges.prepare.nP == None')
            self.pack_uint32(data.pledges.prepare.nP)
            ...
            if data.pledges.confirm.commit is None:
                raise TypeError( 'data.pledges.confirm.commit == None')
            self.pack_SCPBallot(data.pledges.confirm.commit)
            if data.pledges.confirm.nP is None:
                raise TypeError( 'data.pledges.confirm.nP == None')
            self.pack_uint32(data.pledges.confirm.nP)
            ...
            if data.pledges.externalize.nP is None:
                raise TypeError( 'data.pledges.externalize.nP == None')
            self.pack_uint32(data.pledges.externalize.nP)
    ...
    def pack_TransactionSet(self, data):
        ...
        if len(data.txs) > const.MAX_TX_PER_LEDGER and self.check_array:
            raise XDRError( 'array length too long for data.txs')
    ...
    def pack_TransactionResultSet(self, data):
        ...
        if len(data.results) > const.MAX_TX_PER_LEDGER and self.check_array:
            raise XDRError( 'array length too long for data.results')
    ...
    def pack_LedgerEntryChangeType(self, data):
        ...
        if self.check_enum and data not in [const.LEDGER_ENTRY_CREATED, const.LEDGER_ENTRY_UPDATED, const.LEDGER_ENTRY_REMOVED]:
    ...
    def pack_LedgerEntryType(self, data):
        ...
        if self.check_enum and data not in [const.ACCOUNT, const.TRUSTLINE, const.OFFER]:
    ...
    def pack_Signer(self, data):
        ...
        self.pack_AccountID(data.pubKey)
    ...
    def pack_MessageType(self, data):
        ...
        if self.check_enum and data not in [const.ERROR_MSG, const.HELLO, const.AUTH, const.DONT_HAVE, const.GET_PEERS, const.PEERS, const.GET_TX_SET, const.TX_SET, const.TRANSACTION, const.GET_SCP_QUORUMSET, const.SCP_QUORUMSET, const.SCP_MESSAGE]:
    ...
    def pack_AuthenticatedMessage(self, data):
        ...
        self.pack_uint64(data.sequence)
        if data.message is None:
            raise TypeError( 'data.message == None')
        self.pack_StellarMessage(data.message)
        if data.mac is None:
            raise TypeError( 'data.mac == None')
        self.pack_HmacSha256Mac(data.mac)
    ...
    def pack_OperationType(self, data):
        ...
        if self.check_enum and data not in [const.CREATE_ACCOUNT, const.PAYMENT, const.PATH_PAYMENT, const.MANAGE_OFFER, const.CREATE_PASSIVE_OFFER, const.SET_OPTIONS, const.CHANGE_TRUST, const.ALLOW_TRUST, const.ACCOUNT_MERGE, const.INFLATION]:
    ...
    def pack_ChangeTrustResultCode(self, data):
        ...
        if self.check_enum and data not in [const.CHANGE_TRUST_SUCCESS, const.CHANGE_TRUST_MALFORMED, const.CHANGE_TRUST_NO_ISSUER, const.CHANGE_TRUST_INVALID_LIMIT, const.CHANGE_TRUST_LOW_RESERVE]:
    ...
    def pack_AllowTrustResultCode(self, data):
        ...
        if self.check_enum and data not in [const.ALLOW_TRUST_SUCCESS, const.ALLOW_TRUST_MALFORMED, const.ALLOW_TRUST_NO_TRUST_LINE, const.ALLOW_TRUST_TRUST_NOT_REQUIRED, const.ALLOW_TRUST_CANT_REVOKE]:
...
class STELLARXDRUnpacker(xdrlib.Unpacker):
    ...
    def unpack_SCPStatement(self):
        ...
        if data.pledges.type == const.SCP_ST_PREPARE:
            ...
            data.pledges.prepare.nP = self.unpack_uint32()
        elif data.pledges.type == const.SCP_ST_CONFIRM:
            ...
            data.pledges.confirm.commit = self.unpack_SCPBallot()
            data.pledges.confirm.nP = self.unpack_uint32()
        elif data.pledges.type == const.SCP_ST_EXTERNALIZE:
            data.pledges.externalize = nullclass()
            data.pledges.externalize.commit = self.unpack_SCPBallot()
            data.pledges.externalize.nP = self.unpack_uint32()
    ...
    def unpack_Signer(self):
        data = types.Signer()
        data.pubKey = self.unpack_AccountID()
    ...
    def unpack_AuthenticatedMessage(self):
        ...
        data.sequence = self.unpack_uint64()
        data.message = self.unpack_StellarMessage()
        data.mac = self.unpack_HmacSha256Mac()
        if hasattr(self, 'filter_AuthenticatedMessage'):
            data = getattr(self, 'filter_AuthenticatedMessage')(data)
        return data
