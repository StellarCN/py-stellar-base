...
from stellar_base.exceptions import FederationError, StellarAddressInvalidError


async def federation(address_or_id, fed_type='name', domain=None, allow_http=False):
    ...
    fed_service = await get_federation_service(domain, allow_http)
    if not fed_service:
        raise FederationError('Not a valid federation server.')

    return await _get_federation_info(address_or_id, fed_service, fed_type)


async def _get_federation_info(address_or_id, federation_service, fed_type='name'):
    """Send a federation query to a Stellar Federation service.
    ...
    async with aiohttp.ClientSession() as session:
        try:
            async with session.get(federation_service, params=params) as resp:
                if resp.status == 200:
...
async def get_federation_service(domain, allow_http=False):
    ...
    """
    st = await get_stellar_toml(domain, allow_http)
    if not st:
        return None
    return st.get('FEDERATION_SERVER')


async def get_auth_server(domain, allow_http=False):
...
async def get_stellar_toml(domain, allow_http=False):
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(url) as resp:
                    if resp.status == 200:
                        return toml.loads(await resp.text())
            except ClientError:
                pass
    return None
