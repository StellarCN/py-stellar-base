...
def example_verify_challenge_tx_threshold():
    ...
    tx, tx_client_account_id = read_challenge_transaction(
        challenge_tx, server_keypair.public_key, network_passphrase
    )
    ...
    _, tx_client_account_id = read_challenge_transaction(
        challenge_tx, server_keypair.public_key, network_passphrase
    )
    ...
    if client_account_exists:
        ...
        try:
            verify_challenge_transaction_signed_by_client_master_key(
                signed_challenge_tx, server_keypair.public_key, network_passphrase
            )
