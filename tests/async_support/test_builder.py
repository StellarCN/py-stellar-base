...
from stellar_base.async_support import memo
from stellar_base.async_support.builder import Builder
from stellar_base.async_support.horizon import horizon_testnet, horizon_livenet, Horizon
...
class TestBuilder(object):
    async def test_builder_append_ops(self):
    ...
    async def test_builder_xdr(self, setup, test_data):
        ...
        assert response.get('hash') == await builder.hash_hex()

    async def test_import_from_xdr(self, setup, test_data):
        builder = Builder(secret=test_data.cold_secret, sequence=1, horizon_uri=setup.horizon_endpoint_uri,
                          network=setup.network) \
            .append_manage_data_op("hello", "world") \
            .add_time_bounds({'minTime': 1534392138, 'maxTime': 1534392238})
        await builder.sign()
        old_xdr = await builder.gen_xdr()
        old_hash = await builder.hash()
        new_hash = await Builder(secret=test_data.cold_secret).import_from_xdr(old_xdr).hash()
        assert new_hash == old_hash

    async def test_import_from_xdr_custom_network(self, setup, test_data):
        builder = Builder(secret=test_data.cold_secret, sequence=1, horizon_uri=setup.horizon_endpoint_uri,
        ...
        old_xdr = await builder.gen_xdr()
        old_hash = await builder.hash()
        new_hash = await Builder(secret=test_data.cold_secret, network='CUSTOM_NETWORK').import_from_xdr(old_xdr).hash()
        assert new_hash == old_hash

    async def test_next_builder(self, setup, test_data):
        builder = Builder(secret=test_data.cold_secret, sequence=1, horizon_uri=setup.horizon_endpoint_uri,
                          network=setup.network) \
            .append_manage_data_op("hello", "world")
        await builder.sign()
    ...
    async def test_federation_payment(self, setup):
        ...
        assert await builder_fed.hash() == await builder.hash()

    @mock.patch(
        'stellar_base.federation.requests.get',
        side_effect=mocked_requests_get)
    async def test_federation_payment_not_found_raise(self, setup):
        builder_fed = Builder(address='GBTCBCWLE6YVTR5Y5RRZC36Z37OH22G773HECWEIZTZJSN4WTG3CSOES', sequence=100)
        with pytest.raises(FederationError,
                           match='Cannot determine Stellar Address to Account ID translation via Federation server.'):
            await builder_fed.federation_payment(fed_address="bad*fed-domain.com", amount="100.5", asset_code="XLM")

    async def test_network_and_horizon(self, setup, test_data):
