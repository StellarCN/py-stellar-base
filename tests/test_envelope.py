...
class TestOp:
    ...
    def test_createAccount_min(self, setup):
        ...
        assert (result == self.do(setup.network, op=CreateAccount({
    ...
    def test_payment_min(self, setup):
        ...
        assert (result == self.do(setup.network, op=Payment({
            'source': self.source,
            'destination': self.dest,
            'asset': Asset.native(),
            'amount': self.amount,
    ...
    def test_pathPayment_min(self, setup):
        ...
        assert (result == self.do(setup.network, op=PathPayment({
            'source': self.source,
            'destination': self.dest,
    ...
    def test_manageOffer_min(self, setup):
        ...
        assert (result == self.do(setup.network, op=ManageOffer({
    ...
    def test_createPassiveOffer_min(self, setup):
        ...
        assert (result == self.do(setup.network, op=CreatePassiveOffer({
            'selling': Asset('beer', self.source),
            'buying': Asset('beer', self.dest),
            'amount': "100",
            'price': 3.14159,
    ...
    def test_changeTrust_min(self, setup):
        ...
        assert (result == self.do(setup.network, op=ChangeTrust({
    ...
    def test_allowTrust_shortAsset(self, setup):
        ...
        assert (result == self.do(setup.network, op=AllowTrust({
            'trustor': self.dest,
            'asset_code': 'beer',
            'authorize': True,
        })))
    ...
    def test_accountMerge_min(self, setup):
        ...
        assert (result == self.do(setup.network, op=AccountMerge({
            'destination': self.dest,
        })))
    ...
    def test_manage_data(self, setup):
        ...
        assert (result == self.do(setup.network, op=ManageData({
...
class TestMultiOp:
    ...
    def test_double_create_account(self, setup):
        ...
        assert (result == self.make_envelope(setup.network,
            CreateAccount({
    ...
    def test_mix_2(self, setup):
        if setup.type == 'testnet':
            ...
            result = b''
        assert (result == self.make_envelope(setup.network,
            SetOptions({
                'set_flags': 1
            }),
