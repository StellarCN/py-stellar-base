...
class Helpers:
    ...
    def trust_asset(setup, secret_key, memo_text=None):
        ...
        builder = Builder(secret=secret_key, horizon=setup.horizon_endpoint_uri, network=setup.network)
    ...
    def send_asset(cls, setup, secret_key, address, amount, memo_text=None):
        ...
        builder = Builder(secret=secret_key, horizon=setup.horizon_endpoint_uri, network=setup.network)
        builder.append_payment_op(address, amount, setup.test_asset.code, setup.test_asset.issuer)
        if memo_text:
            builder.add_text_memo(memo_text[:28])  # max memo length is 28
        builder.sign()
        reply = builder.submit()
