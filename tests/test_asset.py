...
class TestAsset:
    ...
    def test_non_native_asset_without_issuer_raise(self, code):
        with pytest.raises(AssetIssuerInvalidError, match="The issuer cannot be `None` except for the native asset."):
            Asset(code)
    ...
    def test_invalid_code(self, code):
        with pytest.raises(AssetCodeInvalidError,
    ...
    def test_invalid_issuer(self, issuer):
        with pytest.raises(AssetIssuerInvalidError,
                           match="The issuer should be a correct public key."):
            Asset('XCN', issuer)
    ...
    def test_equals(self):
        assert Asset('XCN', 'GCNY5OXYSY4FKHOPT2SPOQZAOEIGXB5LBYW3HVU3OWSTQITS65M5RCNY') \
